...
class UserLoginTestCase(APITestCase):
    ...
    def test_without_email(self):
        response = self.client.post(self.url, {'email': '', 'password': password})
        self.assertEqual(404, response.status_code, {'msg': 'please input your email.'})
...
class UserDeactivateTestCase(APITestCase):
    ...
    def test_user_delete(self):
        response = self.response.delete(self.url)
        self.assertEqual(204, response.status_code)
        self.assertFalse(User.objects.filter(id=self.user.id).exists())
        self.assertFalse(Token.objects.filter(user_id=self.user.ud).exists())
